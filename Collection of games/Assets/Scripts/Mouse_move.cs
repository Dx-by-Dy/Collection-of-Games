...
public class Mouse_move : MonoBehaviour
{
    ...
    {
        if (Input.GetMouseButtonDown(0) && Table.GetComponent<Start_connecting_game>().I_in_game())
        {
            if (I_in_mouse == false)
            {
                if (Table.GetComponent<Start_connecting_game>().Mouse_free())
                {
        ...
        if (Input.GetMouseButtonDown(1) && Can_I_rotate() && Table.GetComponent<Start_connecting_game>().I_in_game())
        {
            if (Rotate == false)
            {
                Rotate = true;
